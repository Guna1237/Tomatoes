...
def render(user: dict):
    ...
    user_events = database.get_user_registered_events(user_id)
    saved_announcements = database.get_saved_announcements(user_id)
                ...
                with btn_col1:
                    if status == "Accepted":
                        if st.button("Mark as Picked Up", key="dash_pickup_btn", type="primary"):
                            database.update_logistics_status(latest_active["id"], "Picked Up")
                        ...
                        if st.button("Mark as Delivered", key="dash_deliver_btn", type="primary"):
                            database.update_logistics_status(latest_active["id"], "Delivered")
                            st.success("Delivery completed! Credits transferred.")
                            ui_components.safe_rerun()
